#ifndef __MCUPICO_FLASH_W25QXX_H__
#define __MCUPICO_FLASH_W25QXX_H__
...
private:
     ...
     */
    static constexpr uint32_t BAUDRATE = 1000 * 1000;           // --> default value. (1 Mbps) 
    static constexpr uint32_t BAUDRATE_MIN = 50;                // --> tested value.
    static constexpr uint32_t BAUDRATE_MAX = 40 * 1000 * 1000;  // --> from datasheet. (40 Mbps)
...
public:
    ...
    */
    ...
    /**
     * Get the baud-rate for SPI communication.
     */
    inline uint32_t baudrate() const {
        return _baudrate;
    }

    /**
     * Set the baud-rate for SPI communication.
     * This must be called before `init` method.
     */
    bool baudrate(uint32_t value);
